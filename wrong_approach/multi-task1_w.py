...
from multiTask_dataset_w import MultiTask_Dataset_w
...
class MultiTask1(nn.Module):
    ...
    def forward(self, Eea_fingerprint, Egb_fingerprint, Egc_fingerprint, Ei_fingerprint):
        Eea_FCNN_output = self.Eea_FCNN(Eea_fingerprint)
        Egb_FCNN_output = self.Egb_FCNN(Egb_fingerprint)
        Egc_FCNN_output = self.Egc_FCNN(Egc_fingerprint)
        Ei_FCNN_output = self.Ei_FCNN(Ei_fingerprint)

        concatenated_output = torch.cat([Eea_FCNN_output, Egb_FCNN_output, Egc_FCNN_output, Ei_FCNN_output], dim=1)

        final_output = self.Common_FCNN(concatenated_output)
...
def train(model, optimizer, scheduler, Loss, train_dataloader, device, properties):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        print("Total Loss during training",total_loss.item())
        total_loss.backward()
        optimizer.step()
        scheduler.step()
...
def main(multitask_config):
    ...
    if multitask_config['CV_flag']:
        ...
        for fold, (train_idx, val_idx) in enumerate(splits.split(np.arange(data.shape[0]))):
            ...
            train_dataset = MultiTask_Dataset_w(train_data)
            test_dataset = MultiTask_Dataset_w(test_data)
            ...
            """Train the model"""
            model = MultiTask1(drop_rate=multitask_config['drop_rate']).to(device)
            model = model.double()
            Loss = MaskedLoss()
