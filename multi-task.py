...
class MultiTask(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        fingerprint = outputs.last_hidden_state[:, 0, :]
...
def main(multitask_config):
    ...
    if multitask_config['CV_flag']:
        ...
        splits = KFold(n_splits=multitask_config['k'], shuffle=True,
                       random_state=multitask_config['fold_seed'])  # k=1 for train-test split and k=5 for cross validation
...
if __name__ == "__main__":
    ...
    multitask_config = yaml.load(open("config_multiTask.yaml", "r"), Loader=yaml.FullLoader)
